...
function checkTime() {
	...
	if (Math.abs(drawTimeLimit * 60 - scores.time - 60) <= 0.01 && players.length > 2) {
		if (checkTimeVariable == false) {
			...
			room.sendChat("⌛ Quedan 60 segundos pa q termine el partio !");
		}
	}
...
function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
	...
	for (var i = 0; i < extendedP.length ; i++) {
		if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
			room.sendChat("[PV] ⛔ @" + room.getPlayer(extendedP[i][eP.ID]).name + ", Muevete culiao o te kickeo " + Math.floor(afkLimit / 3) + " segundos, seras kickeado!", extendedP[i][eP.ID]);
		}
...
room.onPlayerTeamChange = function(changedPlayer, byPlayer) {
	...
	if (getAFK(changedPlayer) && changedPlayer.team != Team.SPECTATORS) {
		...
		room.sendChat(changedPlayer.name + " està raja durmiendo !");
		return;
	}
	updateTeams();
	if (room.getScores() != null) {
